...
public class CiddeMainActivity extends Activity {
	            ...
	            switch (position) // determine what TYPE of brick will go here
                   ...
                   default:
                     Toast.makeText(CiddeMainActivity.this,
                           "?unknown option?", Toast.LENGTH_SHORT).show();
	...
	private void doIntentHowTo()	{
		...
		//howToIntent.putExtra("intLetterData", intActivityData); // do not use resources for this
		//startActivityForResult(howToIntent, 4);  // start child activity, if result needed
		startActivity(howToIntent);  // start child activity, result needed
		// 4 is the request code (to determine which activity returned, if more than one)
    ...
    @Override
        protected void onActivityResult(int requestCode, int resultCode, Intent data) {
	    	...
	    	//  If new data was returned, then load it up.
	    	int[] msgFromChildLetterData; // holds INCOMING letter data from child activity
	    	msgFromChildLetterData = data.getExtras().getIntArray("intLetterData"); // do not use resource for this
	    	if((msgFromChildLetterData != null) && (msgFromChildLetterData[0] != -1))  // so long as valid data came back
